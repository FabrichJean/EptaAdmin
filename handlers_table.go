...
func (a *App) handleSaveRecords(w http.ResponseWriter, r *http.Request) {
	...
	for column, indices := range deletesByColumn {
		...
		for _, idx := range indices {
			...
			toLog = append(toLog, pendingActivity{ActionValueDelete, map[string]any{"column": column, "index": idx, "value": old}})
		}
	}
	...
	for _, pa := range toLog {
		a.logActivity(logActivityParams{WorkspaceID: ws.ID, TableID: fresh.ID, UserID: currentUser.ID, Action: pa.action, Details: pa.details})
	}
...
func (a *App) handleAddTableColumn(w http.ResponseWriter, r *http.Request) {
	currentUser := userFromContext(r)
	lang := a.resolveLang(r)
	ws, role, ok := a.loadWorkspaceMembership(w, r, currentUser)
	if !ok {
	...
	col, err := a.store.AddTableColumn(t.ID, key, ColumnTypeText, strings.TrimSpace(req.Description))
	if err != nil {
	...
	a.logActivity(logActivityParams{WorkspaceID: ws.ID, TableID: t.ID, UserID: currentUser.ID, Action: ActionColumnAdd, Details: map[string]any{"key": col.Key, "type": col.Type}})
...
func (a *App) handleDeleteTableColumn(w http.ResponseWriter, r *http.Request) {
	...
	a.logActivity(logActivityParams{WorkspaceID: ws.ID, TableID: t.ID, UserID: currentUser.ID, Action: ActionColumnDelete, Details: map[string]any{
