...
func (a *App) HandleWebhookStatus(w http.ResponseWriter, r *http.Request) {
	webutil.WriteJSON(w, http.StatusOK, a.WebhookDeploy.Snapshot())
}
...
func (a *App) HandleWebhookDeliveryProgress(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		webutil.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "invalid JSON body"})
		return
	}
	...
	webutil.WriteJSON(w, http.StatusOK, map[string]bool{"ok": applied})
}
